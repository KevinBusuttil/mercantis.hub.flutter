...
| Selling | Item, Quotation, SalesOrder, SalesInvoice + line items | ✅ | line-item child DocTypes + Item `brand` link; **Item child tables added** — `Item Supplier` (suppliers) + `UOM Conversion Detail` (uoms) |
| Buying | Supplier, SQ, PO, PurchaseInvoice, Receipt + items | ✅ | Supplier, **Supplier Quotation** (+ items, on `wf-supplier-quotation`, in Purchasing workspace), PO, PurchaseInvoice + items, Receipt (under Fulfilment) |
...
| ManufacturingDerivationService | ✅ | ✅ | `Manufacturing` helpers (BOM explosion, cost rollup, completion Stock Entry) + `ManufacturingDerivationService` (Production Plan → Work Orders on submit; **Work Order → completion Stock Entry on the Completed transition**). Stock-only (Swift posts no GL) |
...
| Dashboards (5) | ✅ | ✅ | 5 dashboards (Home, Finance, Sales, Buying, Inventory) on real data via Core DashboardEngine; monetary KPIs use the new `sum` widget. `HubDashboards` + `dashboardEngineProvider` + `DashboardsScreen`/`DashboardScreen`. **Home, Sales & Inventory workspace cards now fed by their real dashboards** (`home`/`sales`/`inventory` via a generic metric/list card host); mock KPIs retired |
