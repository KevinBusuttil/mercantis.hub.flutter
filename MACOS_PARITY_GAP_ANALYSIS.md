...
| H3 | **Document conversion** | `Modules/Selling/HubDocumentConversion.swift` (274): Quotation→SO→Delivery→Invoice, PO→Receipt→Invoice, Lead→Customer/Quotation, partial-fulfilment "remaining lines" | 🟡 **Logic done** — `hub_document_conversion.dart`: pure `HubDocumentConversion` (Quotation→Sales Order, Sales Order→Delivery Note/Invoice, Delivery Note→Invoice, PO→Receipt/Invoice, Receipt→Invoice, Lead→Customer, Quotation-for-customer) with greedy "remaining lines" partial-fulfilment + a `fulfilledByItem` totaliser; maps to the Flutter Hub's field names (posting_date on invoices/fulfilment, sales_order/purchase_order back-links). Covered by `hub_document_conversion_test.dart`. _Remaining:_ wire "Convert" actions into the document UI (a small engine-backed service to total delivered/billed + save+navigate). | High | L |
