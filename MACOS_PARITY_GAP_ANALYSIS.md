...
| C2 | **Posting substrate** | `Posting/PostingBatch.swift` (180) + `DocumentEngine/UnitOfWork.swift` (104) + in-transaction posting writer; deterministic `POST-<src>-v<n>` ids, status (pending/posted/failed/reversed), reversal linkage, recovery & audit | 🟡 **Substrate done** — `posting_batch.dart`: `PostingBatch`/`PostingStatus`, deterministic ids, `PostingBatchWriter` (idempotent upsert/exists on any `DatabaseExecutor`, ready for in-txn use), `PostingBatchStore` (per-source/per-status/current diagnostics), `posting_batches` table (migration `_v6`). Covered by `posting_batch_test.dart`. _Remaining:_ the `UnitOfWork` atomic-with-submit seam, which threads `ExecutionContext` through `DocumentEngine.save` — depends on **C1**. | High | L |
