...
| H2 | **Accounting Autopilot — compliance** | `Modules/Accounting/`: `ComplianceDocTypes.swift` (106 — TaxFiling/TaxFilingBox), `TaxReturnBuilder.swift` (189), `YearEndCloseBuilder.swift` (119), `BooksLockPolicy.swift` (21) + `Company.books_lock_date`, `OpeningBalanceBuilder.swift` (101), `InvoiceStatusService.swift` (175), `HubAccountResolver.swift` (54), `HubPostingFlow.swift` (53) | 🟡 **Compliance DocTypes done** — new `ComplianceModule` registers `Tax Filing` (period, status Draft→Submitted→Filed, output/input/net tax + taxable-base totals, return-box table) and `Tax Filing Box`; covered by `compliance_module_test.dart`. BooksLockPolicy already shipped with H5. **TaxReturnBuilder done** — pure `TaxReturnBuilder` folds a period's `Tax Transaction`s into output/input/net tax + taxable bases + numbered boxes (reversals net); `TaxReturnService.prepare` gathers the filing's tax-type rows in its date range and writes the totals + box rows back onto the `Tax Filing`. Covered by `tax_return_builder_test.dart`. **OpeningBalanceBuilder done** — builds a balanced Draft `Journal Entry` (voucher type `Opening Entry`) from a set of account opening balances, carrying any imbalance to an Opening Balance Equity line; covered by `opening_balance_builder_test.dart`. **YearEndCloseBuilder done** — closes the P&L accounts (debit income, credit expense) and posts the net profit/loss to a seeded `Retained Earnings` account as a Draft `Closing Entry` JE; covered by `year_end_close_builder_test.dart`. **InvoiceStatusService done** — pure `InvoiceStatus.compute` derives Draft/Unpaid/Partly Paid/Paid/Overdue/Cancelled/Return from doc status + outstanding + due date; `InvoiceStatusService` reads an invoice's status and lists a doctype's overdue invoices as of a date. Covered by `invoice_status_service_test.dart`. _Remaining (next PRs):_ account-resolver/posting-flow helpers (largely covered by the existing ledger spine — audit for gaps), + HU1 compliance views. | High | XL |
