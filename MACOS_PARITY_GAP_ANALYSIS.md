...
| CU2 | **Form-UX layer** | `GenericFormView.swift` (+518), `FormPrerequisites.swift` (91), `LinkPickerField.swift` (+204): field help/placeholder, inline validation, prerequisite nudge, **inline create from picker**, currency-symbol affordance, file picker, module hints | 🟡 **Landed:** C4 metadata (`helpText`/`placeholder` on `FieldDefinition`), field help footnotes + placeholders, inline validation (required/numeric, touched-aware + on-save), and the prerequisite "set up your basics first" banner (`form_field_support.dart`, wired into `GenericFormView`). Covered by `form_field_support_test.dart` + `field_definition_help_test.dart`. _Remaining:_ currency-symbol prefix (needs link-resolve plumbing) + inline-create-from-picker. | High | L |
