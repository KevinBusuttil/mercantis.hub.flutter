...
| H2 | **Accounting Autopilot — compliance** | `Modules/Accounting/`: `ComplianceDocTypes.swift` (106 — TaxFiling/TaxFilingBox), `TaxReturnBuilder.swift` (189), `YearEndCloseBuilder.swift` (119), `BooksLockPolicy.swift` (21) + `Company.books_lock_date`, `OpeningBalanceBuilder.swift` (101), `InvoiceStatusService.swift` (175), `HubAccountResolver.swift` (54), `HubPostingFlow.swift` (53) | 🟡 **Compliance DocTypes done** — new `ComplianceModule` registers `Tax Filing` (period, status Draft→Submitted→Filed, output/input/net tax + taxable-base totals, return-box table) and `Tax Filing Box`; covered by `compliance_module_test.dart`. BooksLockPolicy already shipped with H5. _Remaining (next PRs):_ `TaxReturnBuilder` (aggregate Tax Transactions → boxes), `OpeningBalanceBuilder`, `YearEndCloseBuilder`, `InvoiceStatusService`, account-resolver/posting-flow helpers, + HU1 compliance views. | High | XL |
