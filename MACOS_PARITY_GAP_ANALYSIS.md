...
| # | Gap | Swift | Flutter status | Sev | Eff |
|---|-----|-------|----------------|:---:|:---:|
| H1 | **Banking & reconciliation** | `Modules/Banking/`: `BankingDocTypes.swift` (140 — BankAccount/BankStatementLine/BankReconciliation), `BankMatchingService.swift` (127), `BankStatementCSVImporter.swift` (168) | 🟡 **DocTypes done** — new `BankingModule` registers `Bank Account` (mapped to its GL `Account`), `Bank Statement Line` (signed `amount`, Unreconciled/Reconciled/Ignored status, matched-voucher link, import batch), and `Bank Reconciliation` (statement vs ledger balance + difference); covered by `banking_module_test.dart`. _Remaining (next PRs):_ CSV statement importer, match service (lines → Payment Entry / Journal Entry), reconciliation flow + UI. | High | L |
